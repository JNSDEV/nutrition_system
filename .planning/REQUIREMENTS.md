...
- [ ] **CMD-03**: User can run `/weekly-plan` and get a generated meal plan for the upcoming week, drawn from `library/meals.md` + `library/recipes.md`, respecting the 4-portion meal-prep convention and the cycling calendar's load profile.
- [ ] **CMD-04**: User can run `/shopping-list` and get a derived weekly shopping list for the active weekly plan, normalized against the pantry baseline in `templates/shopping-list.md`.
